...
export class TouchControls {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    if (portrait) this.createPortraitGameBoyLayout()
    else this.createLandscapeHandheldLayout()
  }
  ...
  private createLandscapeHandheldLayout() {
    ...
    shell.strokeRoundedRect(wingWidth - 2, 10, width - wingWidth * 2 + 4, height - 20, 24)
  ...
  private createDpad(x: number, y: number, scale: number) {
    ...
    const updateDpad = (pointer: Phaser.Input.Pointer) => {
      ...
      if (distance < deadZone) {
        ...
        if (this.actionPointerId === undefined) this.accelerateDown = false
        setVisualOffset(0, 0)
        return
      }
      ...
      if (this.actionPointerId === undefined) this.accelerateDown = ny < -0.34
    ...
    const release = (pointer?: Phaser.Input.Pointer) => {
      if (pointer && this.dpadPointerId !== undefined && pointer.id !== this.dpadPointerId) return

      this.releaseDpad()
      cross.setAlpha(1)
      centre.setScale(1)

      this.scene.tweens.add({ targets: [cross, shadow], x: 0, y: 0, duration: 85, ease: 'Back.Out' })
      this.scene.tweens.add({ targets: centre, x, y, duration: 85, ease: 'Back.Out' })
      arrows.forEach((arrow, index) => {
        const [ax, ay] = arrowSpecs[index]
  ...
  private createAButtonWithDrift(x: number, y: number, scale: number) {
    ...
    track.fillRoundedRect(x - radius * 0.78, driftTop, radius * 1.56, y - driftTop + radius * 0.78, radius * 0.72)
    track.lineStyle(2, 0x777269, 0.65)
    track.strokeRoundedRect(x - radius * 0.78, driftTop, radius * 1.56, y - driftTop + radius * 0.78, radius * 0.72)
    ...
    const driftArrows = this.scene.add
      .text(x, y - 88 * scale, '▲\n▲', {
        fontFamily: 'monospace',
    ...
    const shadow = this.scene.add.circle(x + 5 * scale, y + 7 * scale, radius, 0x28121f, 0.75).setDepth(80).setScrollFactor(0)
    const button = this.scene.add.circle(x, y, radius, 0x8f285b, 1).setStrokeStyle(5 * scale, 0x3c142b, 1).setDepth(81).setScrollFactor(0)
    const highlight = this.scene.add.circle(x - 8 * scale, y - 10 * scale, radius * 0.72, 0xb74277, 0.38).setDepth(82).setScrollFactor(0)
    ...
    const pressButton = (drifting: boolean) => {
      ...
      driftArrows.setAlpha(drifting ? 1 : 0.7)
    ...
    const release = (pointer?: Phaser.Input.Pointer) => {
      if (pointer && this.actionPointerId !== undefined && pointer.id !== this.actionPointerId) return

      this.releaseActionGesture()
      track.setAlpha(1)
      driftArrows.setAlpha(0.7)
      driftLabel.setColor('#5b5760')
      shadow.setAlpha(0.75)

      this.scene.tweens.add({ targets: [button, label], x, y, scaleX: 1, scaleY: 1, duration: 80, ease: 'Back.Out' })
      this.scene.tweens.add({
        targets: highlight,
        x: x - 8 * scale,
        y: y - 10 * scale,
    ...
    this.track(track, driftLabel, driftArrows, shadow, button, highlight, label, zone)
  }

  private createBButton(x: number, y: number, radius: number) {
    const shadow = this.scene.add.circle(x + 4, y + 6, radius, 0x28121f, 0.72).setDepth(80).setScrollFactor(0)
    const button = this.scene.add
      ...
      .setScrollFactor(0)
      .setInteractive({ useHandCursor: false })
    const highlight = this.scene.add.circle(x - radius * 0.13, y - radius * 0.16, radius * 0.72, 0xb44474, 0.34).setDepth(82).setScrollFactor(0)
    const label = this.scene.add
      .text(x, y, 'B', {
        fontFamily: 'monospace',
  ...
  private createStartButton(x: number, y: number, scale: number) {
    const shadow = this.scene.add.ellipse(x + 2, y + 4, 72 * scale, 28 * scale, 0x000000, 0.35).setDepth(79).setScrollFactor(0)
    const button = this.scene.add.ellipse(x, y, 72 * scale, 28 * scale, 0x494641, 1).setStrokeStyle(2, 0x262522, 1).setDepth(80).setScrollFactor(0)
  ...
  private createSpeakerGrille(x: number, y: number, scale: number) {
    ...
    for (let row = 0; row < 4; row += 1) {
      for (let col = 0; col < 4; col += 1) {
        grille.fillCircle(x + (col - 1.5) * gap, y + (row - 1.5) * gap, radius)
      }
    }
    this.track(grille)
  }

...
export class Mode7Renderer {
  ...
  constructor(scene: Phaser.Scene, sourceTextureKey: string, width: number, height: number, x: number, y: number) {
    this.width = width
    this.height = height
    this.originX = x
    this.originY = y
    ...
    const sourceImage = sourceTexture.getSourceImage() as CanvasImageSource & { width: number; height: number }
    ...
    const outputTexture = scene.textures.createCanvas('mode7-ground', width, height)
    if (!outputTexture) throw new Error('Could not create Mode 7 canvas texture')
    ...
    const contactRow = Phaser.Math.Clamp(this.height - KART_CONTACT_OFFSET_FROM_BOTTOM, 0, this.height - 1)
    this.groundContactDistance = this.distanceForRow(contactRow)
    this.halfFovTangent = Math.tan(Phaser.Math.DegToRad(62 / 2))

    scene.add.image(x, y, 'mode7-ground').setOrigin(0).setDisplaySize(width, height)
  }
  ...
  render(camera: Mode7CameraState) {
    ...
    for (let screenY = 0; screenY < this.height; screenY += 1) {
      ...
      for (let screenX = 0; screenX < this.width; screenX += 1) {
        const lateral = ((screenX / Math.max(1, this.width - 1)) * 2 - 1) * halfWorldWidth
        ...
        if (sourceX < 0 || sourceY < 0 || sourceX >= this.sourceWidth || sourceY >= this.sourceHeight) {
  ...
  projectWorldPoint(worldX: number, worldY: number, camera: Mode7CameraState): Mode7ProjectedPoint | undefined {
    const forwardX = Math.sin(camera.angle)
    const forwardY = -Math.cos(camera.angle)
    const rightX = Math.cos(camera.angle)
    const rightY = Math.sin(camera.angle)
    ...
    const rowProgress = (denominator - this.nearDistance) / (this.farDistance - this.nearDistance)
    const screenY = rowProgress * Math.max(1, this.height - 1)
    const normalizedX = lateral / halfWorldWidth
    const screenX = ((normalizedX + 1) * 0.5) * Math.max(1, this.width - 1)
    return {
  ...
  private distanceForRow(screenY: number) {
    ...
    const denominator = this.nearDistance + (this.farDistance - this.nearDistance) * rowProgress
